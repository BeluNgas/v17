...
BearKey = settings.BearKey
BearKeyy = settings.BearKeyy
...
try {
		...
		const dfrply = fs.readFileSync('./bear.jpg')
		cmhit.push(command)
        mess = {
		...
		const katalog = (teks) => {
             res = helga.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© King Of Bear_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             helga.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = helga.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288279268363-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/IlpQRKWDZed0GgVHmYJUbl', "groupName": `${NamaBot}`, "footerText": "*_© King Of Bear_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
await helga.setStatus(`❄️Base ${BearKey} | ⛈️Running Time : ${uptime}`).catch((_) => _);
      settingstatus = new Date() * 1;
		...
		const pporigi = fs.readFileSync('./bear.jpg')
...
const finv = {
	...
	"message": {
		"groupInviteMessage": {
			"groupJid": "6288279268363-1616169743@g.us",
...
const daftar1 = `Hai kak  ${pushname} ${tampilUcapan} \n\nSebelum Mengakses Bot Verify Terlebih Dahulu Ya, Bot Ini Di Susun Oleh King Of Bear`
const daftar2 = '```Ketik Tombol Di Bawah Untuk Mendaftar Ke Database King Of Bear```'
...
helga.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© King Of Bear_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
...
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*YukiBot*", [{buttonId: 'igm', buttonText: {displayText: 'Instagram'}, type: 1},{buttonId: 'ytm', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
reply('http://instagram.com/drak_ipul123')
}
if(isButton == 'ytm'){
reply('https://www.youtube.com/channel/UCjX0Qvy1y3j8L0GPeH4OoEw')
}
...
Arigatou Gozaimasu! Untuk kalian user ramah dan Beberapa orang yg ikut membantu juga dalam project pembuatan YukiBot-
😖🙏

For GitHub : https://github.com/SaipulAnuar
YouTube ${BearKey} : ${BearKeyy}
`
sendButLocation(from, `${menu}`, `*YukiBot*\n*${BearKey}*`, {jpegThumbnail:ofrply,name:""}, [{buttonId:`ping`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
{buttonId:`sewa`,buttonText:{displayText:'SEWABOT'},type:1}
]
...
case 'sewa':
case 'sewabot':
let bear=`┏━━━ꕥ〔 *SEWA BOT* 〕ꕥ━⬣
┃✾ *Dana :* 0882-7926-8363
┗━ꕥ
┏━━━ꕥ〔 *PEMBAYARAN* 〕ꕥ━⬣
┃✾ *1 Minggu :* Rp 5.000
┃✾ *1 Bulan :* Rp 15.000
┃✾ *1 Permanen :* Rp 30.000
┗━ꕥ`
reply(`${bear}\n\n_*©${BearKey}'s*_`)
break
case 'store':
...
case 'allmenu':
  if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
        ptod = "6288279268363@s.whatsapp.net"
        stod = `${sender}`
        uwu = '```'
...
case 'menu':
        ...
        case '??':
        if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
		quotesnya = await fetchJson(`https://api.lolhuman.xyz/api/random/quotes?apikey=${lolkey}`)
...
helga.sendMessage(from, sound, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
sendButLocation(from, `${menu}`, `${uwu}Base : ${BearKey}\nBase Ini Sedang Dalam Tahap Pengembangan, Gunakan Bot Dengan Sebaik Mungkin!${uwu}\n${timeWib} Wib`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}allmenu`,buttonText:{displayText:'MENU🌸'},type:1},{buttonId:`${prefix}owner`,buttonText:{displayText:'AUTHOR🍁'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
...
case 'bugreport':
              ...
              helga.sendMessage('6288279268363@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
              case 'verify':
...
helga.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./bear.jpg')})
break
case  'blowjob':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
case 'nsfw':
			...
			if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					...
					if (Number(args[0]) === 1) {
						...
						helga.sendMessage(from, `YukiBot`, text)
...
case 'loli':
       ...
       if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              ...
              buttonsMessage = {footerText:'YukiBot', imageMessage: imageMsg,
...
case 'sc':
case 'script':
let bearr = `┏━━━ꕥ〔 *SCRIPT BOT* 〕ꕥ━⬣
┃✾ Bot Ini Script Dari Channel _*King Of Bear*_
┃✾ Bot Ini Base Dari Channel _*Helga Ilham*_
┗━ꕥ`
reply(`${bearr}\n_*Link :*_ ${BearKeyy}\n\n_*©${BearKey}'s*_`)
break
case 'setallmenu':
			...
			case 'ping':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				...
				if (!isOwner) return reply(mess.only.ownerB)
					...
					if (err) return helga.sendMessage(from, `root @King Of Bear:~ ${err}`, text, { quoted: mek })
                 break
                 case 'toimg':
				case 'tomedia':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
...
Giliran = @${tty.player1.split('@')[0]}`
 helga.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     helga.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Subscribe Channel King Of Bears*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer YukiBot",body:"",previewType:"Helga Base",thumbnail:fs.readFileSync('./bear.jpg'),sourceUrl:"https://wa.me/6288279268363"}}})
	}
    }
    }
    }
