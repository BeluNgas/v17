...
//nocache
nocache('./bear.js', module => console.log(color('|UPDATE BASE|'), color(`${module} Updated!`, 'red')))
...
const starts = async (helga = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('KingOfBear Base', {
		font: 'Standard',
	...
	console.log(color('\n> GitHub : ','red'), color(`SaipulAnuar`,'white'))
	console.log(color('> YouTube : ','red'), color(`Saipul Anuar`,'white'))
console.log(color('> TikTok : ','red'), color(`@Raraharsita2`,'white'))
console.log(color('> Instagram : ','red'), color(`@drak_ipul23`,'white'))
console.log(color('\n\n\n Big Thanks To\n\n','orange'), color(`Allah SWT\nMy Parents\nMy Family\nSaipul Anuar\nHelga Ilham (Base Ori)`,'purple'))
	helga.browserDescription = [ 'Base KingOfBear', 'ubuntu', '3.0' ]

	helga.on('qr', () => {
	console.log(color('KingOfBear','yellow'), color('New','yellow'), color('Base','yellow'), color('[ Scan QR Nya ]'))
})
	helga.on('credentials-updated', () => {
		fs.writeFileSync('./session.json', JSON.stringify(helga.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'Base KingOfBear')
	})
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   helga.on('chat-update', async (mek) => {
        require('./bear.js')(helga, mek)
        ownerNumber = ["6288279268363@s.whatsapp.net","6288279268363@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6288279268363@s.whatsapp.net"
...
contentText: `${masuk}`,
footerText: `𝑪𝒓𝒆𝒂𝒕𝒆𝒅 𝑩𝒚 ٬࿊⃟𝑲𝑰𝑵𝑮 𝑶𝑭 𝑩𝑬𝑨𝑹シ︎`, 
buttons: gbutsan,
headerType: 4 }
			helga.sendMessage(mdata.id, buttonMessages, MessageType.buttonsMessage, {thumbnail: fs.readFileSync('./bear.jpg'), "contextInfo": { mentionedJid: [num]}, caption: 'Tes', quoted: fkontakk})
...
helga.on('CB:action,,call', async json => {
        ...
        helga.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./bear.jpg'),sourceUrl:`https://wa.me/6288279268363?text=Assalamualaikum`}}})
        await sleep(5000)
